...
def main() -> int:
    p = argparse.ArgumentParser(description="xdma bram smoke test")
    ...
    print(f"[test] write {args.size} bytes @ 0x{args.offset:x}")
    xfer_write(args.h2c, args.offset, pattern)
    print(f"[test] read  {args.size} bytes @ 0x{args.offset:x}")
    got = xfer_read(args.c2h, args.offset, args.size)

    if got != pattern:
        for i, (a, b) in enumerate(zip(pattern, got)):
            if a != b:
                print(f"[test] mismatch at {i}: expect=0x{a:02x} got=0x{b:02x}")
                break
